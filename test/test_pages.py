...
class PagesTest(unittest.TestCase):
    def setUp(self):
        self.blog_path = path.temppath('tech')
        self.site_path = path.temppath('site')
        os.makedirs(self.blog_path)

        with open(os.path.join(self.blog_path, 'foo.htm'), 'w') as f:
            f.write('Foo')
        with open(os.path.join(self.blog_path, 'bar.htm'), 'w') as f:
            f.write('Bar')
        with open(os.path.join(self.blog_path, '2018-01-01-foo.htm'), 'w') as f:
            f.write('Foo')
        with open(os.path.join(self.blog_path, '2018-01-02-bar.htm'), 'w') as f:
            f.write('Bar')
        with open(os.path.join(self.blog_path, 'header-foo.htm'), 'w') as f:
            f.write('<!-- tag: foo -->Foo')
        with open(os.path.join(self.blog_path, 'header-bar.htm'), 'w') as f:
            f.write('<!-- title: bar -->Bar')
        with open(os.path.join(self.blog_path, 'placeholder-foo.htm'), 'w') as f:
            f.write('<!-- title: foo -->{{ title }}:{{ author }}:Foo')
        with open(os.path.join(self.blog_path, 'placeholder-bar.htm'), 'w') as f:
    ...
    def test_pages_undated(self):
        src = os.path.join(self.blog_path, '[fb]*.htm')
        dst = os.path.join(self.site_path, '{{ slug }}.htm')
        print(dst)
        tpl = '<div>{{ content }}</div>'
        makesite.make_pages(src, dst, tpl)
        with open(os.path.join(self.site_path, 'foo.htm')) as f:
            self.assertEqual(f.read(), '<div>Foo</div>')
        with open(os.path.join(self.site_path, 'bar.htm')) as f:
            self.assertEqual(f.read(), '<div>Bar</div>')

    def test_pages_dated(self):
        src = os.path.join(self.blog_path, '2*.htm')
        dst = os.path.join(self.site_path, '{{ slug }}.htm')
        tpl = '<div>{{ content }}</div>'
        makesite.make_pages(src, dst, tpl)
        with open(os.path.join(self.site_path, 'foo.htm')) as f:
    ...
    def test_content_header_params(self):
        ...
        # post.
        src = os.path.join(self.blog_path, 'header*.htm')
        dst = os.path.join(self.site_path, '{{ slug }}.htm')
        tpl = '{{ title }}:{{ tag }}:{{ content }}'
        makesite.make_pages(src, dst, tpl)
        with open(os.path.join(self.site_path, 'header-foo.htm')) as f:
            self.assertEqual(f.read(), '{{ title }}:foo:Foo')
        with open(os.path.join(self.site_path, 'header-bar.htm')) as f:
    ...
    def test_content_no_rendering(self):
        ...
        # by default.
        src = os.path.join(self.blog_path, 'placeholder-foo.htm')
        dst = os.path.join(self.site_path, '{{ slug }}.htm')
        tpl = '<div>{{ content }}</div>'
        makesite.make_pages(src, dst, tpl, author='Admin')
        with open(os.path.join(self.site_path, 'placeholder-foo.htm')) as f:
    ...
    def test_content_rendering_via_header(self):
        ...
        src = os.path.join(self.blog_path, 'placeholder-bar.htm')
        dst = os.path.join(self.site_path, '{{ slug }}.htm')
        tpl = '<div>{{ content }}</div>'
        makesite.make_pages(src, dst, tpl, author='Admin')
        with open(os.path.join(self.site_path, 'placeholder-bar.htm')) as f:
    ...
    def test_rendered_content_in_summary(self):
        ...
        src = os.path.join(self.blog_path, 'placeholder*.htm')
        post_dst = os.path.join(self.site_path, '{{ slug }}.htm')
        list_dst = os.path.join(self.site_path, 'list.htm')
        post_layout = ''
